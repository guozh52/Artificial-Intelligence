...
class QLearningAgent(BaseAgent):
    def __init__(self, build_env: Callable[[bool], gym.Env], params: Dict[str, Any]):
        ...
        self.policy = np.zeros((self.env.observation_space.n, self.env.action_space.n)) # Q table
    ...
    def get_action(self, state: npt.NDArray, train: bool) -> int:
        ...
        if train and np.random.rand() <= self.eps:
            ...
            # Purely greedy mannar

            if np.count_nonzero(self.policy[state, :]) == 0:
    ...
    def learn(
        ...
        temporal_difference = reward + self.gamma * np.max(self.policy[next_state, :]) - self.policy[state, action]
    ...
    def run(self, max_episodes: int, max_steps: int, train: bool):
        ...
        for episode_ in range(max_episodes):
            ...
            #self.eps = self.min_eps + (0.9 - self.min_eps) * np.exp(-self.eps_decay * episode_)
            if self.eps > self.min_eps:
                self.eps = self.eps * self.eps_decay
            # Decaying learning rate
            #self.lr = self.min_lr + (0.1 - self.min_lr) * np.exp(-self.lr_decay * episode_)
            if self.lr > self.min_lr:
                self.lr = self.lr * self.lr_decay

            episode_rewards[episode_] = total_reward
            print(f"episode {episode_}: {total_reward}")

            print("eps:", self.eps, "lr:", self.lr)
        return episode_rewards
